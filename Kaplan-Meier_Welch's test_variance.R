...
#PRODUZINDO GRÁFICOS BOXPLOTS
#para representar os dados

#Boxplots para o tamanho corporal

instarII_b <- read_excel("body_size.xlsx", sheet = 1, col_names = T)
attach(instarII_b)
instarIII_b<-read_excel("body_size.xlsx", sheet = 2, col_names = T)
attach(instarIII_b)
instarIV_b<-read_excel("body_size.xlsx", sheet = 3, col_names = T)
attach(instarIV_b)
instarV_b<-read_excel("body_size.xlsx", sheet = 4, col_names = T)
attach(instarV_b)
instarVI<-read_excel("body_size.xlsx", sheet = 5, col_names = T)
attach(instarVI_b)
boxplot(c(instarII_b, instarIII_b, instarIV_b, instarV_b, instarVI_b), range=T,notch=F, ylim=c(1.8, 7.5), xlab="Instar", ylab="Prosoma length (mm)")



#Boxplots para a duração do desenvolvimento

instarII_d <- read_excel("developmental_time.xlsx", sheet = 1, col_names = T)
attach(instarII_d)
instarIII<-read_excel("developmental_time.xlsx", sheet = 2, col_names = T)
attach(instarIII_d)
instarIV<-read_excel("developmental_time.xlsx", sheet = 3, col_names = T)
...
instarV<-read_excel("developmental_time.xlsx", sheet = 4, col_names = T)
attach(instarV_d)
boxplot(c(instarII_d, instarIII_d, instarIV_d, instarV_d), range=T, notch=F, ylim=c(), xlab="Instar", ylab="Developmental time (days)")
